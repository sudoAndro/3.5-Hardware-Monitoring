...
class Weather:
    ...
    def stats():
        ...
        wicon_theme_data = weather_theme_data.get('ICON', {})
        ...
        activate = True if wtemperature_theme_data.get("SHOW") or wfelt_theme_data.get(
            ...
            "SHOW") or whumidity_theme_data.get("SHOW") or wicon_theme_data.get("SHOW") else False
        ...
        if activate:
            ...
            if HW_SENSORS in ["STATIC", "STUB"]:
                ...
                if api_key:
                    # Use free "Current Weather" API (2.5): works with standard API keys,
                    # no One Call 3.0 subscription required
        ...
        if activate:
            ...
            Weather.display_icon(wicon_theme_data, icon_code)

    @staticmethod
    def display_icon(theme_data, icon_code):
        if not theme_data.get("SHOW", False):
            return

        x = theme_data.get("X", 0)
        y = theme_data.get("Y", 0)
        width = theme_data.get("WIDTH", 64)
        height = theme_data.get("HEIGHT", 64)

        icon = None
        if icon_code:
            # Icons from OpenWeatherMap are cached locally after first download
            cache_dir = config.MAIN_DIRECTORY / "cache" / "weather-icons"
            os.makedirs(cache_dir, exist_ok=True)
            icon_file = cache_dir / f"owm_{icon_code}@2x.png"
            if not icon_file.exists():
                try:
                    r = requests.get(f"https://openweathermap.org/img/wn/{icon_code}@2x.png", timeout=10)
                    if r.status_code == 200:
                        with open(icon_file, "wb") as f:
                            f.write(r.content)
                except Exception as e:
                    logger.warning(f"Weather icon download failed: {e}")
            if icon_file.exists():
                try:
                    icon = Image.open(icon_file).convert("RGBA").resize((width, height))
                except Exception as e:
                    logger.warning(f"Weather icon could not be loaded: {e}")

        # Compose icon over theme background so transparency looks right
        background_image = get_theme_file_path(theme_data.get("BACKGROUND_IMAGE", None))
        if background_image:
            base = Image.open(background_image).convert("RGB").crop((x, y, x + width, y + height))
        else:
            base = Image.new("RGB", (width, height), (0, 0, 0))
        if icon is not None:
            base.paste(icon, (0, 0), icon)
        display.lcd.DisplayPILImage(base, x, y, width, height)
