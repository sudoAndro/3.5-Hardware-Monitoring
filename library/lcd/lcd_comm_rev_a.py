...
class LcdCommRevA(LcdComm):
    ...
    def Reset(self):
        ...
        # The rev. A serial protocol has no framing: if a previous process was killed mid-command
        # or mid-bitmap, the display controller is stuck waiting for bytes and/or misaligned by
        # 1-5 bytes, and will silently ignore all further commands until it is reset.
        # 1. Padding (multiple of 6 bytes, larger than a full frame) completes any pending bitmap.
        # 2. The RESET command is then sent in all 6 possible byte alignments - exactly one of
        #    them hits the real command frame, whatever the controller's current offset is.
        try:
            self.lcd_serial.write(bytes(310806))
            self.lcd_serial.flush()
            for _ in range(6):
                self.lcd_serial.write(bytes([0, 0, 0, 0, 0, Command.RESET]))
                self.lcd_serial.write(bytes([0]))  # advance alignment by one byte
            self.lcd_serial.flush()
        except Exception as e:
            # The port may die the moment the reset executes: that is expected
            logger.debug("Reset unstick sequence ended with: %s" % e)
        self.closeSerial()
    ...
    def ScreenOff(self):
        ...
        # Some clone displays (e.g. Temu "UsbPCMonitor" 3.5") ignore SCREEN_OFF but
        # honor TO_BLACK (blanks the panel content) and SET_BRIGHTNESS (backlight to
        # minimum): together the display appears fully off on those units
